...
export default function Signup() {
    ...
    async function handleSubmit(e) {
        ...
        try {
            ...
            // We might want to save the user document here later
            navigate("/");
        } catch (err) {
        ...
        <div className="flex flex-col items-center justify-center min-h-screen p-4 overflow-hidden relative">
            ...
            <div className="glass-card w-full max-w-sm space-y-8 animate-fade-in relative z-10">
                <div className="text-center space-y-2">
                    <h2 className="text-3xl font-light tracking-tight text-white">Begin</h2>
                    <p className="text-sm text-zinc-500">Commit to discipline.</p>
                </div>
                ...
                <div className="text-center text-sm text-zinc-500">
                    Already have an account? <Link to="/login" className="text-white hover:underline underline-offset-4 transition-all">Sign In</Link>
                </div>
            </div>
        </div>
    );
